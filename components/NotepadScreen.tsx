...
export const NotepadScreen: React.FC<NotepadScreenProps> = ({ onBack }) => {
        ...
        try {
            ...
            if (dataStr) {
                ...
                setHistory([{
                    ...
                    noteConnections: data.noteConnections || {},
                    sections: loadedSections
                }]);
                const loadedAnnotations = data.annotations || {};
                Object.values(loadedAnnotations).forEach(pageAnns => {
                    pageAnns.forEach(a => {
                        if (!a.id) a.id = generatePathId();
                    });
                });
                const loadedNotes = data.stickyNotes || {};
                Object.values(loadedNotes).forEach(pageNotes => {
                    pageNotes.forEach(n => {
                        if (!n.controlPoints) n.controlPoints = [];
                    });
                });
                setAnnotations(loadedAnnotations);
                setStickyNotes(loadedNotes);
                setNoteConnections(data.noteConnections || {});
                setHistoryIndex(0);
                ...
                if (loadedSourceData) {
                    if (data.sourceType === 'IMAGE') {
                        setSourceData(loadedSourceData as string);
                        setNumPages(1);
                        setPageNum(1);
                    } else {
                        if (typeof loadedSourceData === 'string') {
                            ...
                            setSourceData(loadedSourceData as ArrayBuffer);
                        }
                    }
                } else {
                    setSourceData(null);
    ...
    // UPDATED HANDLE PASTE FOR TYPE SAFETY
    const handlePaste = async () => { 
        try { 
            ...
            for (const item of items) { 
                ...
                if (imageType) { 
                    ...
                    reader.onload = (e) => { 
                        ...
                        if (target?.result && contextMenu) { 
                            addNoteAt(contextMenu.x, contextMenu.y, true, 'image', { mediaUrl: target.result as string }); 
                        } 
                    }; 
                    ...
                    if (text && contextMenu) { 
                        addNoteAt(contextMenu.x, contextMenu.y, true, 'text', { text }); 
                    } 
                } 
            } 
        } catch (err: any) { 
            ...
            try { 
                ...
                if (text && contextMenu) addNoteAt(contextMenu.x, contextMenu.y, true, 'text', { text }); 
            } catch (e: any) { 
    ...
    return (
        <div className="flex h-screen bg-[#f0f4f8] overflow-hidden font-sans text-gray-800">
                ...
                <div className="fixed z-[100] bg-white border border-gray-200 shadow-2xl rounded-lg p-1 text-sm flex flex-col min-w-[220px] animate-pop origin-top-left overflow-hidden" style={{ top: contextMenu.y, left: contextMenu.x }} onMouseDown={(e) => e.stopPropagation()}>
                    ...
                    {contextMenu.type === 'canvas' ? (
                        <>
                            ...
                            <div className="px-3 py-2 bg-gray-50 border-t border-b border-gray-100 text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">Rich Media</div>
                            <button className="w-full text-left px-3 py-2 hover:bg-indigo-50 text-gray-700 font-medium flex items-center gap-3" onClick={() => handleContextMenuAction('upload_image')}><Icon.Image size={14} className="text-gray-400"/> Upload Image</button>
                            <button className="w-full text-left px-3 py-2 hover:bg-indigo-50 text-gray-700 font-medium flex items-center gap-3" onClick={() => handleContextMenuAction('create_image_placeholder')}><Icon.Image size={14} className="text-gray-400"/> Image Placeholder</button>
                            <button className="w-full text-left px-3 py-2 hover:bg-indigo-50 text-gray-700 font-medium flex items-center gap-3" onClick={() => handleContextMenuAction('add_audio')}><Icon.Mic size={14} className="text-gray-400"/> Audio Note</button>
                            <button className="w-full text-left px-3 py-2 hover:bg-indigo-50 text-gray-700 font-medium flex items-center gap-3" onClick={() => handleContextMenuAction('add_table')}><Icon.Table size={14} className="text-gray-400"/> Data Table</button>
                        ...
                        <>
                            <div className="px-3 py-2 bg-gray-50 border-b border-gray-100 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Note Actions</div>
                            <div className="flex gap-1 px-3 py-2">
                                {NOTE_COLORS.map(c => (
                                    <button key={c} className="w-5 h-5 rounded-full border border-gray-200 hover:scale-125 transition-transform" style={{ backgroundColor: c }} onClick={() => handleContextMenuAction('color', c)} />
                                ))}
                            </div>
                            <button className="w-full text-left px-3 py-2 hover:bg-indigo-50 text-gray-700 font-medium flex items-center gap-3" onClick={() => handleContextMenuAction('add_child')}><Icon.Plus size={14} className="text-gray-400"/> Add Child Note</button>
                            <button className="w-full text-left px-3 py-2 hover:bg-indigo-50 text-gray-700 font-medium flex items-center gap-3" onClick={() => handleContextMenuAction('add_anchor')}><Icon.Connect size={14} className="text-gray-400"/> Link to PDF Location</button>
                            <div className="h-px bg-gray-100 my-1"/>
                            <button className="w-full text-left px-3 py-2 hover:bg-red-50 text-red-600 font-medium flex items-center gap-3" onClick={() => handleContextMenuAction('delete')}><Icon.Trash size={14}/> Delete Note</button>
                        </>
                    ) : contextMenu.type === 'annotation' ? (
                        <>
                            <div className="px-3 py-2 bg-gray-50 border-b border-gray-100 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Annotation</div>
                            <div className="flex gap-1 px-3 py-2">
                                {['#ef4444', '#f59e0b', '#10b981', '#3b82f6'].map(c => (
                                    <button key={c} className="w-5 h-5 rounded-full border border-gray-200 hover:scale-125 transition-transform" style={{ backgroundColor: c }} onClick={() => handleContextMenuAction('change_color', c)} />
                                ))}
                            </div>
                            <button className="w-full text-left px-3 py-2 hover:bg-indigo-50 text-gray-700 font-medium flex items-center gap-3" onClick={() => handleContextMenuAction('toggle_arrow')}><Icon.Arrow size={14} className="text-gray-400"/> Toggle Arrowhead</button>
                            <div className="h-px bg-gray-100 my-1"/>
                            <button className="w-full text-left px-3 py-2 hover:bg-red-50 text-red-600 font-medium flex items-center gap-3" onClick={() => handleContextMenuAction('delete_annotation')}><Icon.Trash size={14}/> Delete Annotation</button>
                        </>
                    ) : contextMenu.type === 'connection' ? (
                        <>
                            <div className="px-3 py-2 bg-gray-50 border-b border-gray-100 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Connection</div>
                            <div className="flex gap-1 px-3 py-2">
                                {LINK_COLORS.map(c => (
                                    <button key={c} className="w-5 h-5 rounded-full border border-gray-200 hover:scale-125 transition-transform" style={{ backgroundColor: c }} onClick={() => handleContextMenuAction('connection_color', c)} />
                                ))}
                            </div>
                            <div className="flex gap-1 px-3 py-2">
                                <button onClick={() => handleContextMenuAction('connection_style', 'straight')} className="px-2 py-1 text-[10px] border rounded hover:bg-gray-100">STR</button>
                                <button onClick={() => handleContextMenuAction('connection_style', 'curved')} className="px-2 py-1 text-[10px] border rounded hover:bg-gray-100">CRV</button>
                                <button onClick={() => handleContextMenuAction('connection_style', 'orthogonal')} className="px-2 py-1 text-[10px] border rounded hover:bg-gray-100">90°</button>
                            </div>
                            <button className="w-full text-left px-3 py-2 hover:bg-indigo-50 text-gray-700 font-medium flex items-center gap-3" onClick={() => handleContextMenuAction('add_point')}><Icon.Plus size={14} className="text-gray-400"/> Add Control Point</button>
                            <div className="h-px bg-gray-100 my-1"/>
                            <button className="w-full text-left px-3 py-2 hover:bg-red-50 text-red-600 font-medium flex items-center gap-3" onClick={() => handleContextMenuAction('delete_link')}><Icon.Trash size={14}/> Delete Connection</button>
                        </>
                    ) : contextMenu.type === 'controlPoint' ? (
                        <>
                            <div className="px-3 py-2 bg-gray-50 border-b border-gray-100 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Control Point</div>
                            <button className="w-full text-left px-3 py-2 hover:bg-red-50 text-red-600 font-medium flex items-center gap-3" onClick={() => handleContextMenuAction('delete_point')}><Icon.Trash size={14}/> Delete Point</button>
                        </>
                    ) : null}
                </div>
            )}
            ...
            <NotepadExportModal 
                ...
                elementId="notepad-content-layer"
                projectName={title}
                stickyNotes={Object.values(stickyNotes).flat()}
